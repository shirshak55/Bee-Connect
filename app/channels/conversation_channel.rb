  ...
  def send_message(data)
    ...
    if conversation&.participates?(current_user)
      ...
      if current_user.id == personal_message.conversation.author_id
        Notification.find_or_initialize_by(recipient: personal_message.conversation.receiver, actor: current_user, action: 'messaged', notifiable: personal_message.conversation).update!(read_at: nil)
      end
      if current_user.id == personal_message.conversation.receiver_id
        Notification.find_or_initialize_by(recipient: personal_message.conversation.author, actor: current_user, action: 'messaged', notifiable: personal_message.conversation).update!(read_at: nil)
      end
    end
  end
end
